...
That's the whole integration. Three exports total: `RegisterContract` (once), `FinishHeist` (whenever the heist ends), and optionally `ApplyCooldown` if the contract has a `cooldown` set.
...
opts = {
    cancel = function(ctx)
        ...
        for member, _ in pairs(cc.Groups.GetMembers(ctx.group)) do
...
AddEventHandler('cool_bank:complete', function(group, success)
    if success then
        local cc = exports.cc_lib:GetLib()
        for member, _ in pairs(cc.Groups.GetMembers(group)) do
...
<Note>
  If you need to award bonus VICE or XP outside of what's in the contract's `reward` (e.g. for a hidden objective), call `_G.HeistProfile.AddVice(src, amount, note)` or `_G.HeistProfile.AddXp(src, amount)` directly, then `_G.HeistSync.SendProfile(src)`. Note that this reaches into internals — the public contract is the `reward` table on registration.
</Note>
...
-- Hook the escrowed heist's completion event.
AddEventHandler('cool_bank:complete', function(group, success)
    exports.cc_heistcontracts:FinishHeist(group, success)
end)
```
...
**What if the escrowed heist doesn't fire a completion event?** That's the one case you genuinely can't wrap around. Players' slots will get stuck "active" because nothing ever calls `FinishHeist`. Fall back to one of the [stuck-slot recovery options](/cc_heistcontracts/admin#recovering-a-stuck-active-slot), or ask the escrowed heist's author to add an event.
