...
Directly invokes a contract's entry point, bypassing the slot/purchase/VICE flow, the active-slot tracking, **and the cooldown gate**. Still exported for back-compat with old wrapper scripts (the previously-shipped `/heist:test*` debug commands have been removed), but new integrations should use the dashboard-driven slot flow. A contract started this way has no active slot, so `FinishContract` will return `'not_active'` for the player.
