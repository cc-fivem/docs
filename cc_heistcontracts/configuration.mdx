...
There's no `openKey` and no chat command — the dashboard is opened by using the **`heist_tablet`** (Contracts Tablet) inventory item. The item's use handler calls the `exports('heist_tablet', ...)` export in `client/main.lua`, which runs `openDashboard()`. See [Installation → Register the heist_tablet item](/cc_heistcontracts/installation) for how this is wired up.
