...
<Steps>
  ...
  <Step title="Register the heist_tablet item">
    ...
    ['heist_tablet'] = {
        ...
        weight = 2800,
        stack = false,
        client = { image = 'heist_tablet.png', export = 'cc_heistcontracts.heist_tablet' },
    },
    ```

    The `client.export` field points at `exports('heist_tablet', ...)` in `client/main.lua`, which calls `openDashboard()`. If you're on a different inventory, wire the item's "use" handler to call `exports.cc_heistcontracts:heist_tablet()` on the client.
  </Step>
