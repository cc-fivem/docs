...
The dashboard surfaces these as `error` toasts with the literal error code in the message, so a player seeing `Pickup failed: expired` knows exactly what happened. Buy and start errors have friendlier message mappings — see the [error toasts table](/cc_heistcontracts/dashboard#error-toasts).
