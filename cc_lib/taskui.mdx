...
local listId = cc.TaskUI.Create({
    ...
    onTaskComplete = function(id, taskId) print('task complete', taskId) end,
...
| `onTaskComplete` | `function(id, taskId)` | — | Fires when an objective transitions into `status == 'complete'`. Won't re-fire if `SetStatus('complete')` is called on an already-complete task. |
...
Every transition into `'complete'` fires `onTaskComplete(listId, taskId)`. Setting `'complete'` on the last remaining objective also triggers `onComplete` once.
